...
2. Setup [WhiteNoise](https://whitenoise.readthedocs.io/en/latest/) & collectstatic
   1. Create `static/` in the root folder along with `css/`, `js/` & `img/` subfolders containing .keep files.
   2. `pip install whitenoise`
   3. Add its middleware as mentioned in the [docs](https://whitenoise.readthedocs.io/en/latest/)
   4. In `settings.py`,
      1. Set `STATICFILES_DIRS = [BASE_DIR / "static"]`
      2. Set `STATIC_ROOT = BASE_DIR / 'staticfiles'`
      3. Set `STATICFILES_STORAGE = "whitenoise.storage.CompressedManifestStaticFilesStorage"` or in this case
            ```py
            STORAGES = {
                "default": {
   ...
   6. Add `{% load static %}` to the top of the base.html file.
